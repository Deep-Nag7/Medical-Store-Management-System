...
<head>
  ...
  if (isset($_POST['safeIn'])) {
    if (isset($_FILES['inPic']) && $_FILES['inPic']['error'] == UPLOAD_ERR_OK) {
        $filename = $_FILES['inPic']['name'];
        move_uploaded_file($_FILES["inPic"]["tmp_name"], "photo/" . $_FILES["inPic"]["name"]);
        $pic = $filename;
    } else {
        $pic = NULL; // or provide a default value if necessary
    }

    if ($con->query("INSERT INTO categories (name, pic) VALUES ('$_POST[name]', '$pic')")) {
  ...
  if (isset($_POST['saveProduct'])) {
    // Assuming you have the pic field in inventeries table
    $pic = isset($_FILES['productPic']) && $_FILES['productPic']['error'] == UPLOAD_ERR_OK ? $_FILES['productPic']['name'] : NULL;

    if ($con->query("INSERT INTO inventeries (catId, supplier, name, unit, price, description, company, pic) VALUES ('$_POST[catId]', '$_POST[supplier]', '$_POST[name]', '$_POST[unit]', '$_POST[price]', '$_POST[description]', '$_POST[company]', '$pic')")) {
        $notice = "<div class='alert alert-success'>Successfully Saved</div>";
    } else {
  ...
  <div class="content2">
    ...
    <div style="width: 55%;margin: auto;padding: 22px;" class="well well-sm center">
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-group">
          <label for="productPic" class="col-form-label">Product Picture (Optional)</label>
